...
parent=this.getPreviousSibling(parent);if(parent){this.setFocus(parent);var elements=TC.getElementsByTagAndClassName("input","select",parent);for(var i=0;i<elements.length;i++){element=elements[i];if((element.type=="checkbox")||(element.type=="radio")){if(element.disabled)return;if(element.type==="radio"&&element.checked)return;element.checked=!element.checked;evt.preventDefault();return this.select(element,parent);}}}}

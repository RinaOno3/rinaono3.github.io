...
use MT::Util qw( remove_html encode_html is_valid_email is_valid_url is_url epoch2ts trim );
...
sub script_name { MT->config->CommentScript }

sub tags { MT->app->load_core_tags }
...
sub _send_signup_confirmation {
    ...
    MT::Util::Mail->send_and_log( \%head, $body ) or die MT::Util::Mail->errstr();
}
